...
def trainCycle(train, test, i):
    ...
    kf_model.add(Dense(12, input_dim=8, activation='relu', kernel_initializer='normal', kernel_constraint=maxnorm(3)))
    kf_model.add(Dense(8, activation='relu', kernel_initializer='normal', kernel_constraint=maxnorm(3)))
    kf_model.add(Dense(1, activation='sigmoid'))
    kf_model.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy'])
    kf_model.fit(X[train], Y[train], validation_data=(X[test], Y[test]), epochs=150, batch_size=10, verbose=0)
